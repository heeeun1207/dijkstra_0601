...
start_node = 'E13' #사용자 위치라고 가정
distances, previous = dijkstra(graph, start_node)

# 🧯소화기(F1,F2, F3)를 거쳐 출구(D1)로 가는 가장 빠른 방법 찾기
...
# 소화기(F1,F2,F3)를 거치지 않고 출구(D1)로 가는 최단 경로 찾기
fastest_path_without_via = distances.get('D1', float('inf'))

if fastest_path_without_via < fastest_path_with_via:
    print(f"출구(D1)로 가는 최단거리: {fastest_path_without_via:.1f}")
    print("경로:", end=" ")
    path = ['D1']
    current_node = 'D1'
...
# 🧯소화기(F1,F3)를 거쳐 출구(D1)로 가는 최단 경로 출력
if via_node:
    print(f"🧯{via_node}를 거쳐 출구(D1)로 가는 최단 거리: {fastest_path_with_via:.1f}")
    print("🧯포함 경로:", end=" ")
    path = [via_node, 'D1']
